...
use Pear\Cache\Lite\Exceptions\CacheLiteException;
...
class Lite
{
     ...
     */
    protected $_pearErrorMode = CACHE_LITE_ERROR_RETURN;
    ...
    protected $_hashedDirectoryGroup = NULL;

    protected $_cacheFileMode = NULL;

    protected $_cacheFileGroup = NULL;
 ...
 */
     ...
     * @return string
     * @throws CacheLiteException
     */
    public function get(string $id, string $group = self::DEFAULT_GROUP, bool $doNotTestCacheValidity = false)
    {
        $this->_id = $id;
        $this->_group = $group;
        $data = false;
     ...
     *
     * @param mixed $data data to put in cache (can be another type than strings if automaticSerialization is on)
     * @param string|null $id cache id
     * @param string $group name of the cache group
     * @return boolean true if no problem (else : false or a PEAR_Error object)
     * @throws CacheLiteException
     */
    public function save($data, string $id = null, string $group = self::DEFAULT_GROUP)
    {
        if ($this->_caching) {
            if ($this->_automaticSerialization) {
                $data = serialize($data);
     ...
     * @throws CacheLiteException
     */
    public function remove(string $id, string $group = self::DEFAULT_GROUP, bool $checkBeforeUnlink = false)
    {
        $this->_setFileName($id, $group);
     ...
     * @throws CacheLiteException
     */
    public function clean(string $group = self::DEFAULT_GROUP, string $mode = 'ingroup')
    {
     ...
     * @return void
     */
    public function setLifeTime(int $newLifeTime)
    {
     ...
     * @throws CacheLiteException
     */
    function saveMemoryCachingState(string $id, string $group = self::DEFAULT_GROUP)
    {
        if ($this->_caching) {
            $array = array(
     ...
     * @throws CacheLiteException
     */
    public function getMemoryCachingState(string $id, string $group = self::DEFAULT_GROUP, $doNotTestCacheValidity = false)
    {
        if ($this->_caching) {
     ...
     * @throws CacheLiteException
     */
    public function raiseError(string $msg, int $code = 0)
    {
        throw new CacheLiteException($msg, $code);
    }
     ...
     * @throws CacheLiteException
     */
    private function _unlink(string $file)
    {
        if (!@unlink($file)) {
     ...
     * @throws CacheLiteException
     */
    private function _cleanDir(string $dir, string $group = self::DEFAULT_GROUP, string $mode = 'ingroup')
    {
     ...
     * @return void
     */
    private function _touchCacheFile() {
        if (file_exists($this->_file)) {
            @touch($this->_file);
        }
    }
     ...
     * @return void
     */
    private function _memoryCacheAdd(string $data)
    {
     ...
     * @throws CacheLiteException
     */
    private function _write(string $data)
    {
        if ($this->_hashedDirectoryLevel > 0) {
     ...
     * @throws CacheLiteException
     */
    private function _hash(string $data, string $controlType)
    {
        switch ($controlType) {
            case 'md5':
                return md5($data);
